...
Example:
    ...
    CUDA_VISIBLE_DEVICES=1 /home/stu1/.conda/envs/lxl/bin/python train_refine.py \
        --dataset-name p3m10k \
        --model-backbone mobilenetv3 \
        --model-name mattingrefine-mobilenetv3-p3m10k \
        --model-last-checkpoint "checkpoints/mattingbase-mobilenetv3-p3m10k/epoch-49.pth" \
...
parser.add_argument('--dataset-name', type=str, default='p3m10k', choices=DATA_PATH.keys())
parser.add_argument('--background-dataset', type=str, default='bg20k', choices=('backgrounds', 'bg20k'))

parser.add_argument('--model-backbone', type=str, default='mobilenetv3', choices=['resnet101', 'resnet50', 'mobilenetv2', 'mobilenetv3'])
...
parser.add_argument('--model-name', type=str, default='mattingrefine-mobilenetv3-p3m10k')
parser.add_argument('--model-last-checkpoint', type=str, default='checkpoints/mattingbase-mobilenetv3-p3m10k/epoch-49.pth')
...
parser.add_argument('--log-train-loss-interval', type=int, default=5)
parser.add_argument('--log-train-images-interval', type=int, default=500)
parser.add_argument('--log-valid-interval', type=int, default=1)
...
def train():
    ...
    model = MattingRefine(args.model_backbone,
                          ...
                          args.model_refine_kernel_size).to(device)
    
    if args.model_last_checkpoint is not None:
        load_matched_state_dict(model, torch.load(args.model_last_checkpoint, weights_only=True))
    ...
    for epoch in range(args.epoch_start, args.epoch_end):
        ...
        if (epoch + 1) % args.checkpoint_interval == 0:
            torch.save(model.state_dict(), f'checkpoints/{args.model_name}/epoch-{epoch}.pth')
...
def compute_loss(pred_pha_lg, pred_fgr_lg, pred_pha_sm, pred_fgr_sm, pred_err_sm, true_pha_lg, true_fgr_lg):
    true_pha_sm = resize(true_pha_lg, pred_pha_sm.shape[2:])
    true_fgr_sm = resize(true_fgr_lg, pred_fgr_sm.shape[2:])
    ...
    return F.l1_loss(pred_pha_lg, true_pha_lg) + \
           ...
           F.l1_loss(sobel(pred_pha_lg), sobel(true_pha_lg)) + \
           F.l1_loss(sobel(pred_pha_sm), sobel(true_pha_sm)) + \
           F.l1_loss(pred_fgr_lg * true_msk_lg, true_fgr_lg * true_msk_lg) + \
           F.l1_loss(pred_fgr_sm * true_msk_sm, true_fgr_sm * true_msk_sm) + \
           F.mse_loss(resize(pred_err_sm, true_pha_lg.shape[2:]), \
                      resize(pred_pha_sm, true_pha_lg.shape[2:]).sub(true_pha_lg).abs())
...
def random_crop(*imgs):
    ...
    results = []
    for img in imgs:
        img = resize(img, (int(H_src * scale), int(W_src * scale)))
        img = center_crop(img, (H_tgt, W_tgt))
        results.append(img)
    return results
